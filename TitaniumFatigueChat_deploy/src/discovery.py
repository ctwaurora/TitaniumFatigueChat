...
import pandas as pd
from skills.qwen_skill import call_qwen_text
...
def _build_variable_mechanism_table() -> List[Dict]:
    ...
    先尝试 Qwen 抽取，再用规则回退补充，确保每篇文献至少 1-3 条记录。
    """
    ...
    try:
        result = call_qwen_text(prompt, max_tokens=6000, temperature=0.1)
    ...
    # 合并去重（保留 Qwen 的结果，补充 rule 的结果）
    seen_keys = set()
    all_records = []

    for rec in qwen_records + rule_records:
