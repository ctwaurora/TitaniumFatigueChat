...
def load_chunks_for_paper(paper_id: str) -> List[Dict[str, Any]]:
    ...
    with open(path, "r", encoding="utf-8") as f:
        for line in f:
            try:
                c = json.loads(line)
                if c.get("file_hash", "").startswith(paper_id) or paper_id in c.get("source_file", ""):
                    chunks.append(c)
            except Exception:
                pass
    return chunks
        ...
        with st.expander("📤 上传新文献 PDF", expanded=False):
            ...
            if uploaded_file is not None:
                ...
                with st.spinner("正在处理上传的 PDF..."):
                    save_dir = DATA_DIR / "uploaded_papers"
                    save_dir.mkdir(parents=True, exist_ok=True)
                    file_path = save_dir / uploaded_file.name
                    file_path.write_bytes(uploaded_file.getbuffer())
                    result = process_uploaded_pdf(str(file_path))
                    st.cache_data.clear()
            ...
            with rcols[0]:
                if st.button("📊 查看 RAG 状态", key="check_rag_status"):
                    ...
                    if chunk_path.exists():
                        n_chunks = sum(1 for _ in open(chunk_path, "r", encoding="utf-8"))
                    else:
                        n_chunks = 0
                    st.info(f"RAG chunks: {n_chunks} 条")
            with rcols[1]:
                if st.button("🔄 重建全部 RAG", key="rebuild_all_rag"):
                    ...
                    if st.button("✅ 确认重建", key="confirm_rebuild_rag"):
                        with st.spinner("正在重建全部 RAG..."):
                            ...
                            # 清理旧 chunks
                            _save_chunks([])
                            pdf_dir = DATA_DIR / "uploaded_papers"
                            if pdf_dir.exists():
