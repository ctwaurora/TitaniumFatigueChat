...
def run_evidence_extraction(use_qwen: bool = False, force_rebuild: bool = False):
    """
    主入口：运行结构化证据抽取。

    Args:
        use_qwen: 是否使用 Qwen API 进行抽取（否则用基于规则的方法）
    ...
    if use_qwen:
        try:
            from src.api_keys import get_qwen_api_key
            key = get_qwen_api_key()
            if key:
                from src.qwen_usage import QwenClient
                qwen_api = QwenClient(key)
                print("[INFO] Qwen API initialized")
            else:
                print("[WARNING] No API key found, using rule-based extraction")
        except ImportError:
            print("[WARNING] qwen_usage module not available, using rule-based extraction")
    ...
    for paper in papers:
        ...
        # 抽取证据
        evidence_batch = extract_evidence_from_paper(paper, qwen_api)
