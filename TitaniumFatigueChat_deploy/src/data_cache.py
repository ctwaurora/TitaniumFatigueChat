...
def load_evidence_snippets() -> pd.DataFrame:
    path = _csv_path("evidence_snippets.csv")
    if path.exists():
        return pd.read_csv(path, encoding="utf-8-sig", on_bad_lines="skip")
    return pd.DataFrame()
...
def load_rag_index() -> Optional[Any]:
    """加载 RAG 索引（chunks+向量）。返回 None 表示未构建。"""
    chunks_path = DATA_DIR / "chunks.jsonl"
    if not chunks_path.exists():
        return None
    try:
        from skills.rag_skill import keyword_retrieve
        return {"ready": True, "engine": "keyword"}
    except Exception:
        return None
