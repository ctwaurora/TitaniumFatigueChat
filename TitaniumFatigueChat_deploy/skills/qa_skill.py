"""AI Scientist 问答技能：本地文献库 + 联网检索 + Qwen 综合回答。"""
...
from . import library_skill, qwen_skill, rag_skill, search_skill
...
def answer_question_with_online_literature(
    ...
    """AI Scientist 问答：本地证据 + 在线检索 + Qwen 综合回答。
    ...
    Returns:
        {
          "answer": "Qwen 生成的回答（含引用标记和参考文献小节）",
          "local_evidence": [...],
          "online_papers": [...],
          "references": [...]
        }
...
8. 用中文回答。"""
    ...
    # 第五步：调用 Qwen
    try:
        answer = qwen_skill.call_qwen_text(
            prompt=prompt,
        ...
        result["answer"] = f"Qwen 回答生成失败：{str(e)}"
