"""自动文献发现与联网检索技能：OpenAlex / Semantic Scholar API 搜索 + Qwen 筛选。"""
...
from . import library_skill, qwen_skill
...
def score_relevance_with_qwen(
    paper: Dict[str, Any],
    topic: str = DEFAULT_TOPIC,
) -> Tuple[int, str]:
    """调用 Qwen 判断论文与给定主题的相关性。
...
{{"score": <0-100的整数>, "reason": "<评分理由，中文，20-50字>"}}"""
    ...
    try:
        response_text = qwen_skill.call_qwen_text(
            prompt=prompt,
            model=qwen_skill.DEFAULT_MODEL,
            max_tokens=500,
            temperature=0.1,
        )
    except Exception as e:
        return (0, f"Qwen 调用失败：{str(e)}")
...
def search_and_rank_papers(query: str, max_results: int = 10) -> List[Dict[str, Any]]:
    """综合执行：搜索 OpenAlex → 标准化 → Qwen 相关性评分 → 按分数排序。
    ...
    for paper in raw_papers:
        score, reason = score_relevance_with_qwen(paper, DEFAULT_TOPIC)
