...
调用 Qwen 从 PDF 文本中抽取结构化文献卡片，包含全部领域字段。
"""
...
from .qwen_skill import call_qwen


def _get_qwen_content(response: Any) -> str:
    """从 Qwen 返回结果中提取文本内容（兼容 OpenAI 兼容格式）。"""
    ...
    try:
        ...
        raise ValueError(f"Qwen 返回格式异常：{response}")
...
def extract_literature_card(pdf_text: str) -> Dict[str, Any]:
    """调用 Qwen 从 PDF 文本中抽取结构化文献卡片（增强版）。
...
{text_for_analysis}"""
    ...
    try:
        response = call_qwen(messages, model="qwen-turbo")
        content = _get_qwen_content(response)
