...
def check_password():
    ...
    st.markdown("<h1 style='text-align: center;'>🔬 TitaniumFatigueChat</h1>",
                unsafe_allow_html=True)
    st.markdown("<p style='text-align: center; font-size: 1.1rem; color: #888;'>"
                "面向 L-PBF Ti-6Al-4V 疲劳机制发现的领域科研助手系统</p>",
                unsafe_allow_html=True)
    st.divider()

    password = st.text_input("请输入访问密码", type="password", label_visibility="collapsed",
                             placeholder="访问密码")
    ...
    st.divider()
    st.caption("TitaniumFatigueChat — 面向 L-PBF Ti-6Al-4V 疲劳机制发现的领域科研助手系统")
    return False
...
with st.sidebar:
    ...
    if st.button("🧩 研究空白发现", key="nav_gap",
                 use_container_width=True,
                 type="primary" if current_page == "research_gap" else "secondary"):
        st.session_state.page = "research_gap"
        st.rerun()

    if st.button("🧪 假设生成", key="nav_hypothesis",
                 use_container_width=True,
                 type="primary" if current_page == "hypothesis_gen" else "secondary"):
        st.session_state.page = "hypothesis_gen"
        st.rerun()

    if st.button("🧬 实验方案辅助", key="nav_experiment",
                 use_container_width=True,
                 type="primary" if current_page == "experiment_design" else "secondary"):
        st.session_state.page = "experiment_design"
        st.rerun()

    if st.button("📐 公式模型解释", key="nav_formula",
                 use_container_width=True,
...
# ═══════════════════════════════════════════════════════════════════════════
# PAGE: Formula/Equation Explain (公式模型解释)
# ═══════════════════════════════════════════════════════════════════════════

elif current_page == "formula_explain":
    import pandas as pd
    st.markdown("<h1 style='text-align: center;'>📐 公式模型解释</h1>", unsafe_allow_html=True)
    st.markdown("<p style='text-align:center;color:#888;'>疲劳力学公式、Paris 参数、方程模型浏览与解释</p>")
    st.divider()

    # Load equation data
    eq_path = Path("data/equation_parameter_dataset.csv")
    if eq_path.exists():
        eq_df = pd.read_csv(eq_path, encoding="utf-8-sig", on_bad_lines="skip")
        st.subheader(f"方程参数数据 ({len(eq_df)} 条)")

        # Filter controls
        eq_types = ["All"] + sorted(eq_df["equation_or_model"].unique().tolist()) if "equation_or_model" in eq_df.columns else ["All"]
        eq_filter = st.selectbox("按方程类型筛选", eq_types, key="eq_type_filter")
        filtered_eq = eq_df.copy()
        if eq_filter != "All" and "equation_or_model" in filtered_eq.columns:
            filtered_eq = filtered_eq[filtered_eq["equation_or_model"] == eq_filter]

        st.dataframe(filtered_eq.head(20), use_container_width=True, hide_index=True)
    else:
        st.info("equation_parameter_dataset.csv 不存在。请先运行文献库构建流程。")

    # Fatigue equation library
    fatigue_eq_path = Path("data/fatigue_equation_library.csv")
    if fatigue_eq_path.exists():
        feq_df = pd.read_csv(fatigue_eq_path, encoding="utf-8-sig", on_bad_lines="skip")
        with st.expander(f"📚 疲劳方程库 ({len(feq_df)} 条)", expanded=False):
            st.dataframe(feq_df, use_container_width=True, hide_index=True)

    # Paris law validation data
    paris_path = Path("data/paris_law_validation_dataset.csv")
    if paris_path.exists():
        paris_df = pd.read_csv(paris_path, encoding="utf-8-sig", on_bad_lines="skip")
        with st.expander(f"📊 Paris 定律验证数据 ({len(paris_df)} 条)", expanded=False):
            st.dataframe(paris_df, use_container_width=True, hide_index=True)

    # Link to search for formula queries
    st.divider()
    if st.button("🔍 前往智能搜索查询公式参数", use_container_width=True):
        st.session_state.page = "search"
        st.session_state.answer_mode = "research_analysis"
        st.rerun()
