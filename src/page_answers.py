...
def answer_research_analysis(question: str, ind_var: str, dep_var: str) -> str:
    ...
    if has_candidate_vars:
        ...
        if "pore" in iv or "defect" in iv:
            ...
            lines.append("**log₁₀(Nf) = β₀ - β₁·log₁₀(√area) + β₂·D* + β₃·I_polished + β₄·log₁₀(σa) + β₅·D*×I_polished + ε**\n\n")
            lines.append("其中 D* = distance_to_surface / √area（归一化距表面距离）\n\n")
            lines.append("**建议模型比较**:\n")
            lines.append("| 模型 | 变量 | 复杂度 |\n")
            lines.append("|---|---|---|\n")
            lines.append("| A (baseline) | log(σa) | 最低 |\n")
            lines.append("| B | log(σa) + log(√area) | 低 |\n")
            lines.append("| C | log(σa) + log(√area) + D* | 中 |\n")
            lines.append("| D | log(σa) + log(√area) + D* + I_polished + 交互 | 最高 |\n\n")
            lines.append("**评价指标**: R², adjusted R², RMSE, AIC, BIC, 交叉验证误差\n\n")
            lines.append("**预期方向**: Model C > Model B > Model A 说明加入 D* 有额外解释力\n\n")
            lines.append("**否定条件**: 加入 D* 或 surface_state 后性能无提升、系数方向不稳定或外部验证失败\n\n")
        elif "roughness" in iv:
            lines.append("基于当前变量关系，提出以下候选模型：\n\n")
