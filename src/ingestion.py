...
from src.evidence_extraction import run_evidence_extraction
...
def run_ingest() -> Dict[str, Any]:
    ...
    # 条件化证据抽取：生成 evidence_snippets.csv + variable_relation_dataset.csv
    try:
        ev_stats = run_evidence_extraction(use_qwen=False, force_rebuild=True)
        stats["evidence_total"] = len(ev_stats) if ev_stats else 0
        print(f"[Ingest] 条件化证据抽取完成: {stats['evidence_total']} 条证据")
    except Exception as e:
        print(f"[Ingest] 条件化证据抽取跳过: {e}")
...
def _scan_paper_dirs() -> List[Path]:
    """扫描 papers/, early_papers/, followup_papers/ 中的 PDF。"""
    pdf_files = []
    for dir_name in ["papers", "early_papers", "followup_papers", "data/uploaded_papers"]:
...
def _process_single_pdf(pdf_path: Path) -> Dict[str, str]:
    ...
    if is_oos:
        return {"status": "out_of_scope", "path": str(pdf_path)}
    return {"status": "processed", "path": str(pdf_path)}
