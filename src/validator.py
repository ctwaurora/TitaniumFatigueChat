...
import pandas as pd
from skills.qwen_skill import call_qwen_text, call_qwen
...
"""
    ...
    try:
        result = call_qwen_text(prompt, max_tokens=4000, temperature=0.2, stage="gap_diagnosis")
...
def _compute_hypothesis_scores() -> dict:
    ...
    # ── 1. 证据片段 ──
    ev_path = DATA_DIR / "evidence_snippets.csv"
    ev_count = 0
    ev_types = set()
    quant_count = 0
    if ev_path.exists():
...
def _run_baseline_comparison() -> str:
    ...
    eval_result = {}
    try:
        result = call_qwen_text(eval_prompt, max_tokens=4000, temperature=0.2, stage="quality_gate")
        json_match = re.search(r"\{.*\}", result, re.DOTALL)
        if json_match:
...
def _call_llm(prompt: str, stage: str = "baseline_generation") -> str:
    """调用 LLM 回答问题。"""
    try:
        return call_qwen_text(prompt, max_tokens=4000, temperature=0.3, stage=stage)
