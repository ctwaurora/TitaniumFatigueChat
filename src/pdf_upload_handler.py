...
CANDIDATE_DIR = PAPERS_DIR / "candidate_papers"
...
PAPER_INDEX_JSON = DATA_DIR / "paper_index.json"
...
def ensure_dirs():
    """确保所有需要的目录存在。"""
    for folder in set(TYPE_FOLDER_MAP.values()):
...
def process_uploaded_pdf(pdf_content: bytes, original_filename: str) -> Dict[str, Any]:
    """处理上传 PDF 的 10 阶段流水线。
    ...
    # Stage 1: Save PDF
    record = stage_save_pdf(record, pdf_content)

    # Stage 2: Extract text
    record = stage_extract_text(record)

    # Stage 3: Extract metadata
    record = stage_extract_metadata(record)

    # Stage 4: Classify
    record = stage_classify(record)

    # Stage 5: Move PDF to storage folder
    record = stage_move_pdf(record)

    # Stage 6: Write to candidate_papers.csv (always, even if other)
    ...
    # Stage 7: Write to literature_database.csv (条件入库)
    ...
    # Stage 10: Extract evidence (if text available)
    if safe_get(record, "text_extraction_status") in ("success", "partial"):
        record = stage_extract_evidence(record)
